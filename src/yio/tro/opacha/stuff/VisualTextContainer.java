...
public class VisualTextContainer implements ReusableYio{
    ...
    private void applyTextFit() {
        float maxAllowedWidth = position.width - 4 * GraphicsYio.borderThickness;
    ...
    public void updateTextPosition() {
        float y = position.height - 2 * GraphicsYio.borderThickness;

        for (RenderableTextYio renderableTextYio : textList) {
            renderableTextYio.position.y = y;
            y -= 2f * renderableTextYio.height;
            if (renderableTextYio.centered) {
                renderableTextYio.position.x = position.width / 2 - renderableTextYio.width / 2;
            } else {
        ...
        if (textList.size() == 1) {
            ...
            renderableTextYio.position.y = position.height / 2 + renderableTextYio.height / 2;
            renderableTextYio.updateBounds();
        }
    }
