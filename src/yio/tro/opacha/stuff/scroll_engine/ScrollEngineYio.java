package yio.tro.opacha.stuff.scroll_engine;
...
public class ScrollEngineYio {
    ...
    public boolean applyNormalizedPosition(double value) {
        double targetValue = value * limits.getLength();
        double currentValue = (slider.a + slider.b) / 2;
        if (Math.abs(currentValue - targetValue) < 0.01) return false;
        if (Math.abs(speed) > 0.01) return false;
        relocate(0.2 * (targetValue - currentValue));
        hardCorrection();
        return true;
    }
    ...
    public void setFriction(double friction) {
        this.friction = friction * Math.sqrt(RefreshRateDetector.getInstance().multiplier);
    }
