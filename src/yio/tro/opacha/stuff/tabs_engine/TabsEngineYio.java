package yio.tro.opacha.stuff.tabs_engine;

import yio.tro.opacha.stuff.GraphicsYio;
import yio.tro.opacha.stuff.RepeatYio;
import yio.tro.opacha.stuff.factor_yio.FactorYio;
import yio.tro.opacha.stuff.scroll_engine.SegmentYio;
...
public class TabsEngineYio {
    ...
    public void swipeTab(float direction) {
        ignoreFactor.setValues(1, 0);
        ignoreFactor.destroy(1, Math.min(5, Math.abs(10 / direction)));

        giveImpulse(direction * 6 * magnetMaxPower);
    ...
    private void swipe() {
        ...
        if (Math.abs(sliderCenter - touchDownPos) < 4 * magnetCatchDistance) return;
    ...
    private void checkToStretchLimits() {
        if (slider.getLength() >= limits.getLength()) {
            limits.b = limits.a + slider.getLength() + 1;
        }
    }
